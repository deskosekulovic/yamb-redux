...
const game = (state=initState, action) => {
    switch (action.type) {
    case 'ROLL_DICE':
        return {...state, dice: action.data, rollCounter: state.rollCounter+1};
    case 'SELECTED_DICE':
    ...
    case 'HANDLE_INPUT':
        return {...state,   fields: {...state.fields, [Object.keys(action.data.field)[0]]: Object.values(action.data.field)[0],...action.data.res},
            rollCounter: 0,
            dice: [],
    ...
    case 'HANDLE_MOUSEOVER':
        return {...state,
            fields: {...state.fields, [Object.keys(action.data.field)[0]]: Object.values(action.data.field)[0]}
        };
