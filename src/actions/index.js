...
import calculate, { permissionHandler, columnResult, selectedDice } from '../utilities/Functions';
...
export const rollDice = (selected, dice, rollCounter, inputCount, najavljeno, numberOfDice, numberOfFields, najavaAndRucno) => dispatch => {
    if((numberOfFields-inputCount)!==najavaAndRucno || !rollCounter || najavljeno){
        let diceValue=[];
...
export const handleInput = (e, dice, rollCounter, najavljeno, numberOfDice) => dispatch => {
    ...
    column==='najava' && rollCounter===1 && dispatch(getData('HANDLE_NAJAVA', e.target.id));

    (column==='najava' && (rollCounter===1 && field==='kenta' && input || rollCounter!==1) || column!=='najava')
    && dispatch(getData('HANDLE_INPUT', {field:{[e.target.id]: input}, selected: selectedDice(numberOfDice), res}));
};
