...
export function setExtraColumns(rest = {}) {
  ...
  mergedColumns.map(
    column =>
      (columnResults = {
        ...columnResults,
        [Object.keys(column) + '-suma1']: 0,
        [Object.keys(column) + '-suma2']: 0,
        [Object.keys(column) + '-suma3']: 0
      })
  );
  mergedColumns.map(
    column => (bonus = { ...bonus, [Object.keys(column) + '-suma1']: true })
  );
...
const kenta = (dice, rolls) => {
  ...
  if (
    ((unique.reduce(reducer) === 15 || unique.reduce(reducer) === 20) &&
      unique.length === 5) ||
    unique.length === 6
  ) {
...
export const columnResult = (field, value, reset) => {
  ...
  if (row === 'max' || row === 'min' || row === '1') {
    helper[field] = value;
    columnResults[column + '-suma2'] =
      (helper[column + '-max'] - helper[column + '-min']) *
      helper[column + '-1'];
...
export const permissionHandler = (field, rolls, najavljeno, reset) => {
  ...
  object = {
    ...object,
    ['gore-' + rowsToSelect[rowsToSelect.length - goreIndex - 1]]: true
  };
  object = {
    ...object,
    ['krajevi-' + rowsToSelect[odkrajevaIndex1]]: true,
    ['krajevi-' + rowsToSelect[rowsToSelect.length - odkrajevaIndex2 - 1]]: true
  };
  object = {
    ...object,
    ['sredina-' +
    rowsToSelect[rowsToSelect.length - odsredineIndex1 - 1]]: true,
    ['sredina-' + rowsToSelect[odsredineIndex2]]: true
  };
  ...
  if (field && field.split('-')[0] === 'krajevi') {
    rowsToSelect.slice(0, 6).map(row => {
      row === field.split('-')[1] && odkrajevaIndex1++;
    });
    rowsToSelect.slice(8, 13).map(row => {
      row === field.split('-')[1] && odkrajevaIndex2++;
    });
  }
  if (field && field.split('-')[0] === 'sredina') {
    rowsToSelect.slice(0, 7).map(row => {
      row === field.split('-')[1] && odsredineIndex1++;
    });
    rowsToSelect.slice(7, 12).map(row => {
      row === field.split('-')[1] && odsredineIndex2++;
    });
  }
  rowsToSelect.filter(row => {
    if (row !== field)
      return (object = { ...object, ['slobodno-' + row]: true });
  });
  if (rolls === 1) {
    rowsToSelect.filter(row => {
      if (row !== field)
        return (object = { ...object, ['rucno-' + row]: true });
    });
  }
  if (rolls === 1 || najavljeno) {
    rowsToSelect.filter(row => {
      if (row !== field)
        return (object = { ...object, ['najava-' + row]: true });
    });
  }
  object = { ...object, ...done };
